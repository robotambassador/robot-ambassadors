...
mainwin::mainwin(int argc, char *argv[]) {
    ...
    ros::NodeHandle rosNode("qTracker2D-ROS");
    ros::Publisher positionPub = rosNode.advertise<std_msgs::String>("location", 5);
    ...
    std_msgs::String strLocation;
    strLocation.data = string("Robot (X: %d, Y:%d)", robotLocation.x, robotLocation.y);
    positionPub.publish(strLocation);
    ...
    connect(mediaObject, SIGNAL(finished()), SLOT(finished_play()));

}

void mainwin::play() {
    if (actionPlay_Cricket_Song->isChecked()){
        mediaObject->setCurrentSource(Phonon::MediaSource("SRI80.wav"));
        mediaObject->play();
    }
}
...
void mainwin::about() {
    QMessageBox::about(this, tr("About QTracker2D"),
                       tr("<h2>QTracker2d</h2>"
                          "<p>Copyright &copy; 2010 Alexandros Asthenidis, Georgios Petrou, and Jan Wessnitzer"));
}
